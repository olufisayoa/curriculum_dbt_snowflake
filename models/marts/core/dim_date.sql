WITH date_spine AS (
    SELECT
        ...
        ac.ay_start_date,
        ac.ay_end_date
    FROM {{ ref('stg_manual__date_spine') }} d
    CROSS JOIN {{ ref('stg_manual__academic_calendar') }} ac
...
with_term AS (
    ...
    FROM date_spine ds
    LEFT JOIN {{ ref('stg_manual__academic_calendar') }} ac
        ON ds.academic_year = ac.academic_year
    LEFT JOIN {{ ref('stg_manual__academic_week_lookup') }} awl
